...
- 主时间轴最下方可载入一个本地 MP3/WAV 参考音频；它与游戏音源共用全局播放、暂停、停止、定位和时间轴，默认音量为 50%。
- 音符编辑器的“扒谱模式”可调用可选的 Basic Pitch ONNX 后端。分析会先停止试听并在工作线程运行，不会把文件读取或模型推理放进实时音频回调。
- 识别结果先显示为独立候选层，不会自动改变 `TrackState` 或正式音符。只有点击“写入草稿”才会把当前乐器音域内、且未重复的候选转换为普通 `ntype=0` 音符；这一步可撤销，再点击“应用”或“确定”才提交到工程。
- 当前实现面向一个旋律乐器轨道的人工审阅，不做分轨、鼓件映射或自动配器。多声部录音、噪声、混响和乐器叠加仍可能产生漏音或误音。
- 分析结果与帧级证据缓存在用户的 Local AppData 中，不写入工程、曲谱或发布包。可用 `BDO_TRANSCRIPTION_CACHE` 覆盖缓存目录；源音频路径、大小、修改时间或模型参数变化会使旧缓存失效。
...
- Basic Pitch output is an editable candidate layer, not a verified score. The first implementation analyzes the full reference into one selected melodic track and deliberately rejects percussion tracks.
...
Basic Pitch transcription is optional and is not installed by
`requirements-pyside.txt`. To enable the local ONNX backend:

```powershell
powershell -ExecutionPolicy Bypass -File scripts\install_transcription.ps1
```

The installer uses the upstream `basic-pitch` package with its ONNX runtime
path on Python 3.12. Review the optional dependencies and their upstream terms
before distributing a build that includes them.

该脚本只用于源码环境。当前 Windows Standard 单文件发行版会明确排除
Basic Pitch/ONNX 依赖，不能通过在 EXE 旁运行安装脚本来扩展；需要内置扒谱
的发行包应作为单独版本（或独立 sidecar）构建并完成第三方许可复核。
