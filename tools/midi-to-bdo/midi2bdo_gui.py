...
class App(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        # ── File selection ──
        ctk.CTkLabel(frame, text="输入 MIDI：", text_color=BDO['gold_light']).grid(
        ...
        ctk.CTkButton(frame, text="浏览...", command=self._browse, width=80).grid(
            row=row, column=3, **pad)
        frame.columnconfigure(1, weight=1)
        row += 1

        ctk.CTkLabel(frame, text="曲谱文件名：", text_color=BDO['gold_light']).grid(
            row=row, column=0, sticky='e', **pad)
        ...
        self._step_base_label = ctk.CTkLabel(vel, text="基础：")
        ...
        self._step_step_label = ctk.CTkLabel(vel, text="步进：")
        ...
        self._rescale_min_label = ctk.CTkLabel(vel, text="最小：")
        ...
        self._rescale_max_label = ctk.CTkLabel(vel, text="最大：")
        ...
        self._floor_label = ctk.CTkLabel(vel, text="最低力度：")
        ...
        _make_slider(eff, 0, "混响：", self.reverb)
        _make_slider(eff, 1, "延迟：", self.delay)
        _make_slider(eff, 2, "合唱反馈：", self.chorus_fb)
        _make_slider(eff, 3, "合唱深度：", self.chorus_depth)
        _make_slider(eff, 4, "合唱频率：", self.chorus_freq)

        # ── MIDI info ──
        info_container, info = _make_section(frame, "MIDI 信息")
        info_container.grid(row=row, column=0, columnspan=4, sticky='we', pady=(4, 4), padx=4)
        row += 1
        self._info_label = ctk.CTkLabel(info, text="（未载入文件）",
                                        text_color=BDO['text_dim'])
        self._info_label.grid(sticky='w', padx=6, pady=3)

        # ── Instruments ──
        inst_container, self._instrument_frame = _make_section(frame, "乐器分配")
        ...
        ctk.CTkButton(bottom, text="转换为黑色沙漠曲谱", command=self._convert,
    ...
    def _load_owner_id(self):
        path = _native_open(
            title="选择游戏内保存的单音符曲谱文件",
            filetypes=[("黑色沙漠曲谱文件", "*"), ("所有文件", "*.*")])
        if not path:
            return
        try:
            ...
            if owner_id == 0:
                self._owner_id_status.configure(
                    text="未读取到 Owner ID", text_color=BDO['error'])
                return
            ...
            self._owner_id_status.configure(
                text="文件音符过多，请使用单音符曲谱",
                text_color=BDO['error'])
        except Exception as exc:
            self._owner_id_status.configure(
                text=f"错误：{exc}", text_color=BDO['error'])
    ...
    def _browse(self):
        ...
        if self._tempo_changes > 1 and not self.flatten_tempo.get():
    ...
    def _load_midi_info(self, path):
        try:
            ...
            bpm_str = str(bpm)
            if tempo_changes > 1:
                bpm_str += f"（{tempo_changes} 次速度变化）"

            self._info_label.configure(
                text=(f"通道：{len(channel_groups)}  |  音符：{total_notes}  |  "
                      f"BPM：{bpm_str}  |  节拍：{tsig}/4\n"
                      f"时长：{dur_str}  |  音域：{pitch_str}"),
                text_color=BDO['text'])
            ...
            self.status_text.set("文件已载入")
        except Exception as exc:
            self._channel_groups = []
            self._inst_combos = []
            self._info_label.configure(text=f"错误：{exc}",
                                       text_color=BDO['error'])
            self.status_text.set("MIDI 载入失败")
    ...
    def _populate_instruments(self):
        ...
        ctk.CTkLabel(merge_frame, text="全部合并为：",
                     text_color=BDO['text_dim'],
        ...
        ctk.CTkButton(merge_frame, text="应用", width=60,
        ...
        # Column headers
        ctk.CTkLabel(self._instrument_frame, text="来源",
                     text_color=BDO['gold_light'],
                     font=ctk.CTkFont(size=11)).grid(
            row=1, column=0, sticky='w', padx=(6, 8), pady=(2, 4))
        ctk.CTkLabel(self._instrument_frame, text="乐器",
                     text_color=BDO['gold_light'],
                     font=ctk.CTkFont(size=11)).grid(
            row=1, column=2, sticky='w', padx=(4, 6), pady=(2, 4))
        ctk.CTkLabel(self._instrument_frame, text="音量",
                     text_color=BDO['gold_light'],
                     font=ctk.CTkFont(size=11)).grid(
        ...
        for i, (notes, gm_prog, is_perc) in enumerate(self._channel_groups):
            ...
            if is_perc:
                label_text = f"打击乐（通道 10，{len(notes)} 个音符）"
            else:
                label_text = f"{gm_program_name(gm_prog)}（{len(notes)} 个音符）"
    ...
    def _convert(self):
        ...
        if not path or not os.path.isfile(path):
            self.status_text.set("错误：请选择有效的 MIDI 文件")
            return

        out_name = self.output_name.get().strip()
        if not out_name:
            self.status_text.set("错误：曲谱文件名不能为空")
            return
        ...
        try:
            ...
            self.status_text.set("错误：移调必须是整数")
            return
        ...
        if bpm_str:
            try:
                ...
                self.status_text.set("错误：BPM 必须是整数")
                return
        ...
        try:
            ...
            self.status_text.set("错误：力度参数必须是整数")
            return
        ...
        if self._channel_groups and self._inst_combos:
            ...
            for i, (notes, gm_prog, is_perc) in enumerate(self._channel_groups):
                ...
                if selected_name in name_to_id:
                    instrument_map[i] = name_to_id[selected_name]
        ...
        try:
            ...
            status = (f"已保存：{out_path}  ({len(bdo_data)} 字节，"
                      f"{summary['tracks']} 条轨道，{summary['total_notes']} 个音符)")
            if summary.get('notes_dropped', 0):
                status += (f"  警告：已丢弃 {summary['notes_dropped']} 个音符"
                           f"（单乐器 10000 音符限制）")
            self.status_text.set(status)
        except Exception as exc:
            self.status_text.set(f"错误：{exc}")
