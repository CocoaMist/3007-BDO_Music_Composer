...
唯一可以压缩无效音高行的现有乐器是已由游戏谱确认的架子鼓套装：
...
因此这两件乐器现在可以做明确的合法性检查。架子鼓目标轨始终使用语义鼓件
行：原生 BDO 48–64 / `ntype=99` 显示 BDO 鼓件名并可压缩无效行；尚未在导出
投影中转换的 MIDI 通道 10 鼓轨保留原始 GM 音高和 `ntype`，但显示 Kick、
Snare、Hi-Hat、Crash、Ride 等 GM 鼓键名并保持完整滚动范围；活动音符若
超出映射则显示为 `MIDI n · 未映射鼓键` 并标为无效。卷帘由目标乐器自动选择，
不显示手动切换框；显示模式与鼓音规范化相互独立，不得静默改写正式音符。
手碟 `0x13` 仍只有近似
...
`drum_lanes` 替换架子鼓的普通钢琴键标签，并用 `visual_key` 选择背景。任何
