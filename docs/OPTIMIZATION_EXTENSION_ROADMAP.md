...
- `pyside_bdo_gui.py` 已从约 24,540 行降至约 9,500 行，职责从“大型组件定义集合”收敛为主窗口编排和兼容导出。
...
- `model_revision.py` 现提供显式、单调递增的模型 revision；转换校验不再依赖可变列表身份或额外的全曲指纹扫描。
- 扒谱分析、混合审阅历史、工程载入和预览传输状态已分别进入 Qt-free controller/coordinator，主窗口保留兼容属性和 UI/I/O 编排。

## 本轮已落实

`acknowledgements_dialog.py` 现在拥有致谢 HTML、许可证链接、复制动作和完整 Qt 布局。主窗口只注入当前主题并打开对话框。这样许可证展示可独立测试，也避免新增致谢项时触碰主窗口编排。
...
目前 `MidiToBdoWindow` 中最密集的区域仍是扒谱候选路由、和声/声部分组和 UI 同步。worker generation 与 restart 状态已迁入 `TranscriptionAnalysisCoordinator`，混合审阅历史已迁入 `TranscriptionReviewController`；下一步建立 `TranscriptionWorkspaceHost` 协议，再按三段迁移：

1. `TranscriptionReviewController`：混合撤销/重做、eligible/reject/restore 和疑似碎音选择计划已完成；编辑器 route 暂存也改为按 ID 索引读取。下一切片迁移和弦/声部分组 mutation plan，但正式音符提交仍经过现有 Apply/OK gate。
2. `TranscriptionAnalysisCoordinator`：继续迁移启动/取消、worker 结果和缓存恢复；generation/restart 已完成。
3. `TranscriptionWorkspacePresenter`：把领域状态投影成按钮状态、状态文本和可见图层。

完成判据：控制器不导入主窗口；领域决策可在无 Qt 窗口条件下测试；主窗口只连接 signal、选择当前轨和提交用户确认结果。

### P1：工程生命周期控制器

打开 MIDI/BDO、工程快照、自动保存和首页刷新仍由主窗口串联。`ProjectLifecycleController` 已接管 loading generation/gate；下一步迁移类型化 open/save 请求，但保持 `project_persistence.py` 和 `export_workflow.py` 为序列化事实源。不要把 `QWidget` 或可变 `TrackState` 交给后台 writer。

完成判据：打开/保存/恢复的输入输出由类型化请求描述；错误映射和 UI 提示与文件 I/O 分离；现有迁移和隐私测试保持通过。
...
- 结论边界：本轮没有改动 paint/audio callback，也不宣称渲染或 DSP 提速；墙钟绘制值只用于发现退化。下一轮优先迁移候选 route/语义审阅命令或类型化工程 open/save 请求。
...
- 下一步：迁移和弦/声部分组 mutation plan，或转入类型化工程 open/save 请求；不要把候选索引复制到主窗口或 canvas。
