...
`bdo_music_composer/app/application_metadata.py`。所有调用方直接改用 canonical
包路径，根目录不保留 shim；根 Python 文件约束由 69 降到 52。长期把根目录
收敛到 7–10 个文件仍只是演进方向，不能在文档或实现中宣称已经完成。

## 1. 先找所有者，再改调用者

`pyside_bdo_gui.py` 是 Qt 组合根和兼容门面，不是默认的领域逻辑所有者。历史
...
| 正式谱面/试听作用域、最终游戏乐器 ID、同乐器 Volume/Aux | `game_score_model.py` | UI 发出意图，预览/验证/导出消费同一规则 |
| 游戏默认轨道音量及 FX wire 值 | `bdo_track_effects.py` | 其他模块引用命名常量，不复制 `70` 等游戏语义值 |
| 转换设置与音高计划 | `conversion_settings.py`, `pitch_transform.py` | 设置界面替换不可变快照，不重建字典规则 |
...
| 首页目录数据与首页/启动展示 | `home_catalog.py`, `bdo_music_composer/ui/home_widgets.py`, `bdo_music_composer/ui/startup_widgets.py` | 扫描器不创建控件；展示组件不反向导入主窗口 |
...
| 完整工程文档校验与加载计划 | `bdo_music_composer/project/project_document.py` | UI 只读取文本、注入端口、本地化错误，并一次性提交 `ProjectLoadPlan` |
...
| 扒谱草稿与候选路由的正式提交计划 | `transcription_commit_plan.py` | UI 做 preflight、单次撤销快照、应用计划、提交 sidecar、刷新和 autosave |
| 不可变导出请求 factory、准备和发布 | `export_workflow.py` | 主窗口只完成路径/Owner/本地化 gate，并传入当前编辑器事实源 |
| 编辑器到 BDO v9 的字段级一致性诊断 | `export_verification.py` | Qt-free 投影并检查 prepared、主文件、游戏副本；UI 只显示有范围限定的报告 |
...
| 游戏规则问题的确定性收集与排序 | `bdo_validation.py` | UI 只本地化/展示 `ValidationIssue`，不复制校验规则 |
...
- `bdo_music_composer/editor/editor_import.py` 和 `game_score_model.py`
  不依赖 PySide、主窗口或任何控件；
- `bdo_codec/` 不依赖编辑器、游戏模型、工程、导出适配器或 UI；
- `bdo_export/` 可以依赖 Codec、MIDI 和 wire-safe 效果值，但不依赖应用工作流、
  工程或 UI；
- `export_workflow.py` 和 `export_verification.py` 可以向下调用
  `bdo_export`/`bdo_codec`，不能回头读取 Qt
  或工程存储；
- `bdo_music_composer/project/` 边界不依赖 Codec、导出层或 Qt；
  `bdo_music_composer/project/project_document.py` 在状态修改前产出完整
...
- `bdo_music_composer/app/application_config.py`、
  ...
  `transcription_commit_plan.py` 保持 Qt-free；主窗口不能直接绕过
  `export_workflow.py` 调用 Codec/导出适配器；
...
- `bdo_music_composer.editor.preview_midi_writer.build_filtered_midi()` 是标准
  MIDI 投影的唯一实现；
  `pyside_bdo_gui.py` 只兼容重导出同一对象。它不属于 BDO v9 导出，也不证明
  游戏内效果可由标准 MIDI 无损表达。
...
4. 在 `pyside_bdo_gui.py` 或控件模块只连接信号、注入展示值并应用已验证结果。
...
- `game_score_model.py` 统一正式谱面、试听和游戏混音身份；
...
- `bdo_music_composer/project/project_document.py` 已将工程 JSON 完整准备成
  `ProjectLoadPlan`，加载失败带稳定 code/path；`ProjectMetadataSnapshot`
  已递归深冻结工程 metadata，保存 worker 不再持有 GUI 可变字典/列表；
- `transcription_commit_plan.py` 已提供确定性、非修改输入的正式提交计划；
