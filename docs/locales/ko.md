...
BDO Music Composer는 Black Desert 악보를 로컬에서 만들고 검토하고 미리 듣고 내보내는 비공식 데스크톱 음악 편집기입니다. 범용 DAW가 아니며 Pearl Abyss와 제휴하지 않습니다.
...
편집, 자동 저장, 최적화, 미리듣기, 채보 지원, 악보 내보내기에는 자동 회귀 검증이 있습니다. 컴퓨터, 오디오 장치, 게임 버전에 따라 호환성이 달라질 수 있습니다.

<!-- section:features -->
## 주요 기능

- MIDI를 가져오거나 빈 프로젝트에서 시작해 여러 트랙의 음표, 벨로시티, 리듬, 주법을 편집합니다.
- 현재 편집 상태를 유지하면서 기존 악보를 열고 다시 내보냅니다.
- 로컬 채보 지원 결과를 확정 전에 편집 가능한 초안으로 검토합니다.
- 취소 가능한 최적화, 자동 저장, 내보내기 검사, 로컬 프로젝트 관리를 제공합니다.
