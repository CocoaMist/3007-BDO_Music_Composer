...
是直接证据，八字节到 RTPC 的最终运行时绑定仍是推断边界。Bank 引用也不等于
游戏 authoring 权限：当前官方音乐指南只对弗洛凯斯特拉与玛勒尼斯乐器声明
Effector/AuxSend，初级乐器的八字节只做无损保留，不据此开放新建编辑或试听。
...
10 ms 基础调制延迟合唱。Delay Feedback 以官方教程声明的约 `2–20` 个延迟声
作为 `0–100` 端点，通过 -40 dB 可听边界生成稳定反馈系数；LFO Frequency 的
0 值使用保守的 `0.03 Hz` 慢速运动而非停振，100 仍使用 bank 上限 `0.3 Hz`。
混响与 Flanger 深度曲线采用上述 bank 范围；feedback 在本地轻量处理器中限制
...
- 时间轴乐器行保留原有的“轨道音量”短滑轨。顶部时间轴工具栏另有一条整首曲谱
  共用的“全局力度基数”滑轨，不读取或修改任何乐器级 `Volume`。它不是百分比，
  而是可在 `-127..127` 调整的有符号整数，零值位于滑块中央。未勾选“均化”
  时，基数直接加到所有正力度后按游戏 `0..127` 边界裁切。勾选“均化”时，先加
  基数，再对整组主、副力度使用同一个比例映射：若正向超过 127，则以调整后的
  最大值为基准整体缩放到 127，例如 `20/50/80 + 100` 的原始结果
  `120/150/180` 映射为 `85/106/127`。负向越界使用对称的下边界映射；同时越过
  两端时映射完整跨度。力度为零的现有音块也参与基数调整，不会令控件失效。
  在当前编辑基线不变期间，每次拖动都从首次捕获的原始主、副力度重新计算，绝不
  从上一次裁切或均化结果累计；因此基数回到 0 时会逐音符精确恢复原始力度。
  普通音符编辑发生后，编辑后的正式力度成为新的基线。
  松手或确认输入后，结果一次性写入
  正式音符，试听、
  工程和 BDO 导出使用同一结果，操作作为一个整体进入撤销；滑块保留在当前基数
  位置，打开其他工程时从 0 开始。
  轨道左侧区域的右键菜单另提供“轨道力度基数…”；它使用同一套 `-127..127`
  基数和可选 B 均化算法，但输入集合只包含所选逻辑轨道的主、副力度。确认后作为
  一次可撤销操作写入该轨道，其他轨道及所有乐器级 Volume 均保持不变。
  “轨道 FX”窗口只承载 Reverb、Delay、Chorus 三个 Aux Send，不修改五个共享主
  效果参数。初级乐器仍可使用顶部全局增益并查看导入的 Aux 状态，但 Aux 控件
  禁用，程序化提交同样不会改变其八字节。
...
1. 在同一游戏版本建立固定单音曲谱，每次只把一个 authoring 控件从
   `0` 改到 `1/25/50/75/100`，保存后解密比较八字节。
2. 把差分谱重新载入游戏并再次保存，确认控件、XML 与 v9 字节稳定往返；同时
   覆盖同乐器 730 音符拆块和多个不同乐器组。
3. 使用用户本地回录测量 Reverb RT60、Delay 间隔/重复包络、Chorus LFO
   周期/深度；录音和游戏资源均不得进入 Git。
4. 只有单变量差分与游戏录音 A/B 同时闭环，才把对应 profile 从
   `uncalibrated` 升级为版本化的 `verified`；此前导出精确性与试听近似性继续
   分开声明。
