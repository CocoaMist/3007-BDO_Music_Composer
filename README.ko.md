...
- 로컬 WAV 또는 표준 MP3를 불러오고 분석을 눌러 Basic Pitch ONNX/CPU 분석을 로컬에서 실행합니다.
- 제품 UI는 보호된 standard/balanced/preserve 설정을 사용하며 실험 매개변수, 프레이즈, 화성, voice group, 편성 진단을 노출하지 않습니다.
- “분석 음표”는 가벼운 윤곽선과 아래쪽 인식 신뢰도 막대로 표시하며 영역 선택, 필터링, 초안 추가가 가능합니다. 읽기 전용 연속성 판정은 근거가 있는 동일 음높이의 약한 분할만 연결하고 각 어택과 후보 ID는 유지합니다. “음높이 선”은 히스테리시스 추적, 제약된 이중 베지어 선, 독립 불투명도와 표시 전용 낮음·표준·높음 노이즈 제거를 사용하는 별도 보기입니다. 자동 음색 분류는 신뢰 가능한 프로토타입을 먼저 만들고 짧은 조각을 흡수한 뒤 분류 수와 신뢰도를 표시합니다. 소유 그룹이 하나인 선만 해당 색을 사용하며 충돌이나 근거 부족은 중립색으로 유지합니다. 선택형 멜로디 안내는 여러 시간 구간에서 수동 편집 음표가 적중한 음색 그룹에 중복 제거 및 상한이 있는 가중치를 추가합니다. 안정되면 해당 그룹의 분석 음표와 음높이 선을 현재 트랙 악기로 최우선 표시하지만 음향 인식이나 내보내기는 바꾸지 않습니다.
