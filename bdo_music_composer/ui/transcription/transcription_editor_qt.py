...
def _responsive_control_width(
    ...
    """Prefer translated content width within the adaptive command deck."""
...
class TranscriptionEditorPanel(QWidget):
    """Practical transcription controls for ``MidiNoteEditorDialog``.

    The semantic-assist widgets remain importable for old project/test tooling,
    but the production editor intentionally exposes only the reliable path:
    choose audio, analyze it, review candidates, and apply a selection.
    """
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._rhythm_diagnostic_busy = False
        self._rhythm_alignment_available = False
        ...
        self._write_action_available = False
        self._reject_action_available = False
        self._review_undo_available = False
        ...
        self._analysis_layout = analysis
        ...
        self.workspace_title_label.setObjectName(
            "TranscriptionWorkspaceTitle"
        )
        analysis.addWidget(self.workspace_title_label, 0, 0)

        self.audio_button = QPushButton(tr("载入音频"), self.analysis_bar)
        self.audio_button.setObjectName("TranscriptionAudioButton")
        self.audio_button.setAccessibleName(tr("载入或更换参考音频"))
        _responsive_control_width(self.audio_button, 64, 104)
        self.audio_button.clicked.connect(self._audio_button_clicked)
        analysis.addWidget(self.audio_button, 0, 1)

        self.remove_audio_button = QPushButton(
            tr("移除音频"),
            self.analysis_bar,
        )
        self.remove_audio_button.setObjectName("TranscriptionRemoveAudioButton")
        self.remove_audio_button.setAccessibleName(tr("移除参考音频"))
        self.remove_audio_button.setToolTip(tr("移除参考音频"))
        _responsive_control_width(self.remove_audio_button, 72, 104)
        ...
        analysis.addWidget(self.analyze_button, 0, 3)
        ...
        analysis.addWidget(self.redecode_button, 0, 4)
        ...
        self.cleanup_profile_mark.hide()
        ...
        self.candidate_layer_button = QToolButton(self.analysis_bar)
        self.candidate_layer_button.setObjectName(
        ...
        self.candidate_layer_button.setPopupMode(
            QToolButton.ToolButtonPopupMode.MenuButtonPopup
        )
        _responsive_control_width(self.candidate_layer_button, 126, 184)
        self.candidate_layer_button.toggled.connect(
            self._candidate_layer_toggled
        )

        self.candidate_opacity_slider = QSlider(
            Qt.Horizontal,
            self.analysis_bar,
        )
        self.candidate_opacity_slider.setObjectName(
            "TranscriptionCandidateOpacitySlider"
        )
        self.candidate_opacity_slider.setRange(0, 100)
        self.candidate_opacity_slider.setValue(52)
        self.candidate_opacity_slider.setFixedWidth(112)
        self.candidate_opacity_slider.setAccessibleName(tr("候选音块透明度"))
        self.candidate_opacity_slider.setToolTip(
            tr("调整全部识别候选音块的透明度")
        )
        self.candidate_opacity_slider.valueChanged.connect(
            self._candidate_opacity_slider_changed
        )
        self.candidate_opacity_label = QLabel("52%", self.analysis_bar)
        self.candidate_opacity_label.setFixedWidth(38)
        ...
        self.melody_lines_button.setText(tr("声部提示"))
        self.melody_lines_button.setAccessibleName(tr("声部提示"))
        self.melody_lines_button.setToolTip(tr("分析后显示短距离声部提示"))
        _responsive_control_width(self.melody_lines_button, 78, 132)
        self.melody_lines_button.setCheckable(True)
        self.melody_lines_button.setChecked(False)
        ...
        for role, label in (
            ...
            action.setChecked(role == "primary_melody")
        ...
        self.pitch_guide_button = QToolButton(self)
        ...
        self.pitch_only_button = QToolButton(self)
        ...
        self.pitch_only_button.toggled.connect(self._pitch_only_toggled)
        self.contour_denoise_combo = QComboBox(self)
        self.contour_denoise_combo.setObjectName(
            "TranscriptionContourDenoiseCombo"
        )
        for label, value in (
            ("去噪：低", "low"),
            ("去噪：标准", "standard"),
            ...
            self.contour_denoise_combo.addItem(tr(label), value)
        self.contour_denoise_combo.setCurrentIndex(
            self.contour_denoise_combo.findData("standard")
        )
        self.contour_denoise_combo.setAccessibleName(tr("音高轨迹去噪等级"))
        self.contour_denoise_combo.setToolTip(
            tr("只清理音高轨迹显示，不修改识别结果、草稿或导出")
        )
        _responsive_control_width(self.contour_denoise_combo, 104, 138)
        ...
        self.rhythm_projection_checkbox = QCheckBox(
            tr("显示整理后"),
            self.analysis_bar,
        )
        self.rhythm_projection_checkbox.setAccessibleName(
            tr("显示节奏整理后的参考音块")
        )
        self.rhythm_projection_checkbox.setToolTip(
            tr("关闭后显示原始识别时间；不会删除节奏整理结果")
        )
        self.rhythm_projection_checkbox.setChecked(True)
        self.rhythm_projection_checkbox.setEnabled(False)
        self.rhythm_projection_checkbox.toggled.connect(
            self.rhythm_projection_enabled_changed
        )
        self.rhythm_profile_combo = QComboBox(self.analysis_bar)
        self.rhythm_profile_combo.setObjectName(
            "TranscriptionRhythmProfileCombo"
        )
        self.rhythm_profile_combo.addItem(tr("自动网格"), "auto")
        self.rhythm_profile_combo.addItem(
            tr("严格 1/64"),
            "strict_1_64",
        )
        ...
        _responsive_control_width(self.rhythm_profile_combo, 92, 128)
        ...
        self.candidate_layer_button.setMenu(self.candidate_layer_menu)

        self.advanced_panel = QFrame(self)
        self.advanced_panel.setObjectName("TranscriptionAdvancedPanel")
        self._advanced_layout = QGridLayout(self.advanced_panel)
        self._advanced_layout.setContentsMargins(8, 5, 8, 5)
        self._advanced_layout.setHorizontalSpacing(10)
        self._advanced_layout.setVerticalSpacing(4)

        self.guide_tools_bar = QFrame(self.advanced_panel)
        self.guide_tools_bar.setObjectName("TranscriptionGuideToolsBar")
        guide_tools = QHBoxLayout(self.guide_tools_bar)
        guide_tools.setContentsMargins(6, 2, 6, 2)
        ...
        analysis.addWidget(self.advanced_toggle_button, 0, 5)
        analysis.setColumnStretch(6, 1)

        guide_tools_label = QLabel(tr("分析辅助"), self.guide_tools_bar)
        guide_tools_label.setObjectName("TranscriptionGroupLabel")
        guide_tools.addWidget(guide_tools_label)
        ...
        self.rhythm_diagnostic_button = QPushButton(
            tr("节奏整理"),
            self.analysis_bar,
        )
        self.rhythm_diagnostic_button.setAccessibleName(tr("节奏整理"))
        self.rhythm_diagnostic_button.setToolTip(
            tr(
                "自动检测速度和第一拍，将参考音块投影到最合适的"
                "1/4–1/64或三连音网格；可随时切回原始识别时间"
            )
        )
        _responsive_control_width(
            self.rhythm_diagnostic_button,
            72,
            112,
        )
        self.rhythm_diagnostic_button.clicked.connect(
            self.rhythm_diagnostic_requested
        )
        ...
        self.alignment_tools_bar = QFrame(self.advanced_panel)
        self.alignment_tools_bar.setObjectName(
            "TranscriptionAlignmentToolsBar"
        )
        alignment_tools = QHBoxLayout(self.alignment_tools_bar)
        alignment_tools.setContentsMargins(6, 2, 6, 2)
        alignment_tools.setSpacing(6)
        alignment_tools_label = QLabel(
            tr("音频校准"),
            self.alignment_tools_bar,
        )
        alignment_tools_label.setObjectName("TranscriptionGroupLabel")
        alignment_tools.addWidget(alignment_tools_label)
        alignment_tools.addWidget(self.align_audio_button)
        alignment_tools.addWidget(self.beat_origin_button)
        alignment_tools.addWidget(self.rhythm_diagnostic_button)
        alignment_tools.addWidget(self.clear_range_button)
        alignment_tools.addStretch(1)

        self.candidate_tools_bar = QFrame(self.advanced_panel)
        self.candidate_tools_bar.setObjectName(
            "TranscriptionCandidateToolsBar"
        )
        candidate_tools = QHBoxLayout(self.candidate_tools_bar)
        candidate_tools.setContentsMargins(6, 2, 6, 2)
        candidate_tools.setSpacing(6)
        candidate_tools_label = QLabel(
            tr("候选音块"),
            self.candidate_tools_bar,
        )
        candidate_tools_label.setObjectName("TranscriptionGroupLabel")
        candidate_tools.addWidget(candidate_tools_label)
        self.candidate_layer_hint = QLabel(
            tr("分析结果 · 尚未写入轨道"),
            self.candidate_tools_bar,
        )
        self.candidate_layer_hint.setObjectName("Muted")
        candidate_tools.addWidget(self.candidate_layer_hint)
        candidate_tools.addStretch(1)
        self.tuning_tools_bar = QFrame(self.advanced_panel)
        self.tuning_tools_bar.setObjectName("TranscriptionTuningToolsBar")
        tuning_tools = QHBoxLayout(self.tuning_tools_bar)
        tuning_tools.setContentsMargins(6, 2, 6, 2)
        tuning_tools.setSpacing(6)
        tuning_tools_label = QLabel(tr("识别参数"), self.tuning_tools_bar)
        tuning_tools_label.setObjectName("TranscriptionGroupLabel")
        tuning_tools.addWidget(tuning_tools_label)
        self.analysis_mode_caption = QLabel(
            tr("场景"),
            self.tuning_tools_bar,
        )
        self.analysis_mode_caption.setBuddy(self.analysis_mode_combo)
        tuning_tools.addWidget(self.analysis_mode_caption)
        tuning_tools.addWidget(self.analysis_mode_combo)
        self.sensitivity_caption = QLabel(
            tr("灵敏度"),
            self.tuning_tools_bar,
        )
        self.sensitivity_caption.setBuddy(self.sensitivity_combo)
        tuning_tools.addWidget(self.sensitivity_caption)
        tuning_tools.addWidget(self.sensitivity_combo)
        tuning_tools.addWidget(self.cleanup_profile_group)
        tuning_tools.addStretch(1)

        self.diagnostic_layers_bar = QFrame(self.advanced_panel)
        self.diagnostic_layers_bar.setObjectName(
            "TranscriptionDiagnosticLayersBar"
        )
        diagnostic_layers = QHBoxLayout(self.diagnostic_layers_bar)
        diagnostic_layers.setContentsMargins(6, 2, 6, 2)
        diagnostic_layers.setSpacing(8)
        diagnostic_label = QLabel(
            tr("诊断证据"),
            self.diagnostic_layers_bar,
        )
        diagnostic_label.setObjectName("TranscriptionGroupLabel")
        diagnostic_layers.addWidget(diagnostic_label)
        for widget in (
            self.frame_checkbox,
            self.onset_checkbox,
            ...
            diagnostic_layers.addWidget(widget)
        diagnostic_layers.addStretch(1)
        ...
        self.review_context_label = QLabel(
            tr("识别结果"),
            self.review_bar,
        )
        self.review_context_label.setObjectName(
        ...
        self.review_display_group.setObjectName(
            "TranscriptionReviewDisplayGroup"
        )
        review_display = QHBoxLayout(self.review_display_group)
        review_display.setContentsMargins(4, 2, 4, 2)
        review_display.setSpacing(4)
        for widget in (
        ...
        self.reject_button = QPushButton(tr("忽略所选"), self.review_bar)
        _responsive_control_width(self.reject_button, 64, 96)
        self.reject_button.clicked.connect(self.reject_requested)
        self.restore_button = QPushButton(tr("恢复忽略"), self.review_bar)
        ...
        self.review_action_group = QFrame(self.review_bar)
        self.review_action_group.setObjectName(
            "TranscriptionReviewActionGroup"
        )
        review_actions = QHBoxLayout(self.review_action_group)
        review_actions.setContentsMargins(4, 2, 4, 2)
        ...
        self.review_commit_group.setObjectName(
            "TranscriptionReviewCommitGroup"
        )
        ...
        review_commit.addWidget(self.staging_label)
        ...
        review_commit.addWidget(self.clear_staging_button)
        ...
        review_commit.addWidget(self.write_current_track_button)

        self.review_tools_toggle_button = QToolButton(self.review_bar)
        self.review_tools_toggle_button.setObjectName(
            "TranscriptionReviewToolsToggle"
        )
        self.review_tools_toggle_button.setText(tr("更多操作"))
        self.review_tools_toggle_button.setAccessibleName(tr("审阅工具"))
        self.review_tools_toggle_button.setToolTip(tr("审阅工具"))
        self.review_tools_toggle_button.setCheckable(True)
        self.review_tools_toggle_button.setArrowType(Qt.RightArrow)
        _responsive_control_width(
        ...
        self.review_more_bar = QFrame(self)
        self.review_more_bar.setObjectName("TranscriptionReviewMoreBar")
        review_more = QHBoxLayout(self.review_more_bar)
        review_more.setContentsMargins(8, 4, 8, 4)
        review_more.setSpacing(6)
        for widget in (
            self.review_undo_button,
            self.review_redo_button,
            self.select_fragments_button,
            self.copy_to_track_button,
        ):
            review_more.addWidget(widget)
        review_more.addStretch(1)
        root.addWidget(self.review_more_bar)
        ...
        self.contour_checkbox.toggled.connect(
            self._sync_pitch_guide_button,
        )
        self.melody_lines_button.toggled.connect(
            self._melody_lines_toggled,
        )
        ...
        self._apply_practical_surface()
        self._refresh_analysis_controls()

    def _apply_practical_surface(self) -> None:
        """Remove expert-only controls from the production interaction path."""

        self.advanced_toggle_button.hide()
        self._analysis_layout.removeWidget(self.advanced_toggle_button)
        ...
        self.redecode_button.hide()
        self.review_tools_toggle_button.hide()
        self.review_more_bar.hide()
        self.select_fragments_button.hide()
        self.copy_to_track_button.hide()
        self.review_undo_button.hide()
        self.review_redo_button.hide()
    ...
    @property
    def rhythm_alignment_profile(self) -> str:
        value = str(self.rhythm_profile_combo.currentData() or "auto")
        return value if value in {"auto", "strict_1_64"} else "auto"

    @property
    def review_tools_expanded(self) -> bool:
        return self._review_tools_expanded
    ...
    def _apply_advanced_responsive_layout(self) -> None:
        ...
        if not hasattr(self, "_advanced_layout"):
            return
        wide = self.width() >= 1280
        if self._advanced_layout_wide == wide:
            return
        self._advanced_layout_wide = wide
        for widget in (
            self.tuning_tools_bar,
            self.candidate_tools_bar,
            self.guide_tools_bar,
            self.alignment_tools_bar,
            self.diagnostic_layers_bar,
        ):
            self._advanced_layout.removeWidget(widget)
        if wide:
            self._advanced_layout.addWidget(self.tuning_tools_bar, 0, 0)
            self._advanced_layout.addWidget(self.candidate_tools_bar, 0, 1)
            self._advanced_layout.addWidget(self.guide_tools_bar, 1, 0)
            self._advanced_layout.addWidget(self.alignment_tools_bar, 1, 1)
            self._advanced_layout.addWidget(
                self.diagnostic_layers_bar,
                2,
                0,
                1,
            ...
            self._advanced_layout.addWidget(self.tuning_tools_bar, 0, 0)
            self._advanced_layout.addWidget(self.candidate_tools_bar, 1, 0)
            self._advanced_layout.addWidget(self.guide_tools_bar, 2, 0)
            self._advanced_layout.addWidget(self.alignment_tools_bar, 3, 0)
            self._advanced_layout.addWidget(
                self.diagnostic_layers_bar,
                4,
                0,
    ...
    def set_advanced_controls_expanded(self, expanded: bool) -> None:
        ...
        self.tuning_tools_bar.setVisible(expanded)
        for widget in (
    ...
    def set_review_tools_expanded(self, expanded: bool) -> None:
        expanded = bool(expanded and self._review_tools_available)
        self._review_tools_expanded = expanded
        blocked = self.review_tools_toggle_button.blockSignals(True)
        self.review_tools_toggle_button.setChecked(expanded)
        self.review_tools_toggle_button.blockSignals(blocked)
        self.review_tools_toggle_button.setArrowType(
            Qt.DownArrow if expanded else Qt.RightArrow
        )
        self.review_more_bar.setVisible(expanded)
    ...
    def set_diagnostic_evidence_expanded(self, expanded: bool) -> None:
        ...
        self.diagnostic_layers_bar.setVisible(
            expanded and self._advanced_controls_expanded
        )
    ...
    def set_assist_available(self, available: bool) -> None:
        # Harmony, phrase, voice grouping, and instrument matching are not part
        # of the practical product surface.  Keep the setter as a compatibility
        # sink for older host code without constructing the expensive widget
        # tree or making the feature discoverable.
        self._assist_available = False
    ...
    def set_phrase_state(
        ...
        *,
        ...
        del index, total, loop_enabled, review_count
    ...
    def set_voice_group_matches(
        ...
        *,
        ...
        del voice_group, matches, confirmed_instrument_id
    ...
    @property
    def candidate_layer_visible(self) -> bool:
        return self.candidate_layer_button.isChecked()

    @property
    def candidate_opacity(self) -> float:
    ...
    def contour_denoise(self) -> str:
        return str(
            self.contour_denoise_combo.currentData() or "standard"
        )
    ...
    def _audio_button_clicked(self) -> None:
        # Clicking a loaded source means "replace", not the destructive and
        # surprising "unload" action.  Removal has its own explicit button.
    ...
    def _candidate_opacity_slider_changed(self, value: int) -> None:
    ...
    def _leave_pitch_only_without_restore(self) -> None:
        if not self.pitch_only_button.isChecked():
            return
        blocked = self.pitch_only_button.blockSignals(True)
        self.pitch_only_button.setChecked(False)
        self.pitch_only_button.blockSignals(blocked)
        self._pitch_only_previous = None

    def _candidate_layer_toggled(self, visible: bool) -> None:
        if visible:
    ...
    def _melody_lines_toggled(self, visible: bool) -> None:
        if visible:
            self._leave_pitch_only_without_restore()
        self.melody_lines_visibility_changed.emit(bool(visible))

    def _sync_pitch_guide_button(self, visible: bool) -> None:
        blocked = self.pitch_guide_button.blockSignals(True)
        self.pitch_guide_button.setChecked(bool(visible))
        self.pitch_guide_button.blockSignals(blocked)
        self.contour_denoise_combo.setEnabled(
            bool(visible) and self.pitch_guide_button.isEnabled()
        )

    def _pitch_guide_button_toggled(self, visible: bool) -> None:
        if not visible and self.pitch_only_button.isChecked():
            self.pitch_only_button.setChecked(False)
        self.contour_checkbox.setChecked(bool(visible))
        self.contour_denoise_combo.setEnabled(
            bool(visible) and self.pitch_guide_button.isEnabled()
        )

    def _pitch_only_toggled(self, enabled: bool) -> None:
        """Switch between a clean raw-pitch view and the prior layer state."""

        if enabled:
            if self._pitch_only_previous is None:
                self._pitch_only_previous = (
                    self.candidate_layer_visible,
                    self.melody_lines_visible,
                    self.contour_checkbox.isChecked(),
                )
            self.candidate_layer_button.setChecked(False)
            self.melody_lines_button.setChecked(False)
            self.pitch_guide_button.setChecked(True)
            return
        previous = self._pitch_only_previous
        self._pitch_only_previous = None
        if previous is None:
            return
        candidate_visible, melody_visible, contour_visible = previous
        self.candidate_layer_button.setChecked(candidate_visible)
        self.melody_lines_button.setChecked(melody_visible)
        self.pitch_guide_button.setChecked(contour_visible)

    def _reference_opacity_slider_changed(self, value: int) -> None:
        self.reference_opacity_label.setText(f"{int(value)}%")
        self.reference_opacity_button.setText(
            trf("参考层 · {opacity}%", opacity=int(value))
        )
    ...
    def set_audio_loaded(
        ...
        *,
        ...
        self.audio_button.setText(
            tr("更换音频") if loaded else tr("载入音频")
        )
    ...
    def set_fragment_state(
        ...
        *,
        ...
        if hasattr(self, "review_tools_toggle_button"):
            self._refresh_contextual_visibility()
    ...
    def set_candidate_layer_visible(self, visible: bool) -> None:
        if visible:
            self._leave_pitch_only_without_restore()
        blocked = self.candidate_layer_button.blockSignals(True)
        self.candidate_layer_button.setChecked(bool(visible))
        self.candidate_layer_button.blockSignals(blocked)

    def set_candidate_opacity(self, value: float) -> None:
        normalized = max(0.0, min(1.0, _finite_float(value, 0.52)))
        slider_value = round(normalized * 100.0)
        blocked = self.candidate_opacity_slider.blockSignals(True)
        self.candidate_opacity_slider.setValue(slider_value)
    ...
    def set_contour_denoise(self, value: str) -> None:
        index = self.contour_denoise_combo.findData(str(value))
        if index < 0:
            index = self.contour_denoise_combo.findData("standard")
        if index < 0 or index == self.contour_denoise_combo.currentIndex():
            return
        blocked = self.contour_denoise_combo.blockSignals(True)
        self.contour_denoise_combo.setCurrentIndex(index)
        self.contour_denoise_combo.blockSignals(blocked)
    ...
    def set_evidence_layers(self, layers: Iterable[str]) -> None:
        ...
        self._sync_pitch_guide_button("contour" in visible)
    ...
    def set_draft_note_count(self, count: int) -> None:
        self._draft_note_count = max(0, int(count))
        has_draft = self._draft_note_count > 0
        self._refresh_contextual_visibility()
    ...
    def set_rhythm_diagnostic_state(
        self,
        *,
        busy: bool,
        ...
        """Render the disposable beat-aware candidate projection state."""

        self._rhythm_diagnostic_busy = bool(busy)
        self._rhythm_alignment_available = aligned_count is not None
        self.rhythm_projection_checkbox.setEnabled(
            self._rhythm_alignment_available and not busy
        )
        if busy:
            self.rhythm_diagnostic_button.setText(tr("节奏整理中…"))
        elif aligned_count is None and proposal_count is None:
            self.rhythm_diagnostic_button.setText(tr("节奏整理"))
            self.rhythm_diagnostic_button.setToolTip(
                tr(
                    "自动检测速度和第一拍，将参考音块投影到最合适的"
                    "1/4–1/64或三连音网格；可随时切回原始识别时间"
        ...
        elif aligned_count is None:
            self.rhythm_diagnostic_button.setText(
                trf(
                    "节奏建议 {count}",
            ...
            self.rhythm_diagnostic_button.setText(
                trf(
                    "已对齐 {count}",
                    count=max(0, int(aligned_count)),
                )
            )
            if detected_bpm is not None and confidence is not None:
                self.rhythm_diagnostic_button.setToolTip(
                    trf(
                        "检测 BPM {bpm} · 置信 {confidence}% · "
                        "点击重新整理；参考音块菜单可切换原始/整理后时间",
                        bpm=f"{float(detected_bpm):.1f}",
    ...
    def set_action_state(
        ...
        *,
        ...
        draft_note_count: int | None = None,
    ) -> None:
        self._write_action_available = bool(write_enabled)
        self._reject_action_available = bool(
            write_enabled if reject_enabled is None else reject_enabled
        )
        self._rejected_count = max(0, int(rejected_count))
        self._review_undo_available = bool(can_undo)
        self._review_redo_available = bool(can_redo)
        if candidate_count is not None:
            self._candidate_count = max(0, int(candidate_count))
        if draft_note_count is not None:
            self._draft_note_count = max(0, int(draft_note_count))
        self.write_current_track_button.setEnabled(
            self._write_action_available
        )
        ...
        self.reject_button.setEnabled(
            self._reject_action_available
        )
        self.restore_button.setEnabled(self._rejected_count > 0)
        self.review_undo_button.setEnabled(self._review_undo_available)
        self.review_redo_button.setEnabled(self._review_redo_available)
    ...
    def _refresh_contextual_visibility(self) -> None:
        """Remove inactive commands from the layout without removing features."""
        ...
        self.alignment_tools_bar.setVisible(
            advanced and (self._audio_loaded or self._range_available)
        )
        self.diagnostic_layers_bar.setVisible(
            advanced
            and has_reference_context
            and self.diagnostic_toggle_button.isChecked()
        )
        self.redecode_button.setVisible(
            self._range_available and not self.PRACTICAL_MODE
        )

        self.review_context_label.setVisible(has_candidates)
        # Keep display-layer controls spatially stable; before analysis they
        # remain visible but disabled by _refresh_analysis_controls().
        self.review_display_group.setVisible(True)
        self.reject_button.setVisible(self._reject_action_available)
        self.restore_button.setVisible(self._rejected_count > 0)
        self.write_current_track_button.setVisible(
            self._write_action_available
        )
        self.staging_label.setVisible(self._staging_count > 0)
        self.clear_staging_button.setVisible(self._staging_count > 0)
        self.review_action_group.setVisible(
            self._reject_action_available or self._rejected_count > 0
        )
        self.review_commit_group.setVisible(
            self._staging_count > 0
            or self._write_action_available
            or self._draft_note_count > 0
        )
        self.review_undo_button.setVisible(self._review_undo_available)
        self.review_redo_button.setVisible(self._review_redo_available)
        self.select_fragments_button.setVisible(
            self._suspected_fragment_count > 0 and not self.PRACTICAL_MODE
        )
        copy_available = bool(
            self._copy_allowed and self.copy_to_track_menu.actions()
        )
        self.copy_to_track_button.setVisible(
            copy_available and not self.PRACTICAL_MODE
        )
        self._review_tools_available = bool(
            self._review_undo_available
            or self._review_redo_available
            or self._suspected_fragment_count > 0
            or self._staging_count > 0
            or copy_available
        )
        self.review_tools_toggle_button.setVisible(
            self._review_tools_available and not self.PRACTICAL_MODE
        )
        if not self._review_tools_available:
            self.set_review_tools_expanded(False)
    ...
    def _refresh_analysis_controls(self) -> None:
        ...
        self.remove_audio_button.setEnabled(
        ...
        self.melody_lines_button.setToolTip(
            tr("默认只提示主旋律；仅连接相邻且可信的短距离音符")
            if melody_lines_enabled
            else tr("分析后显示短距离声部提示")
        )
        guide_available = (
            self._audio_loaded
        ...
        self.candidate_layer_button.setEnabled(guide_available)
        self.pitch_guide_button.setEnabled(guide_available)
        self.pitch_only_button.setEnabled(guide_available)
        self.contour_denoise_combo.setEnabled(
            guide_available and self.pitch_guide_button.isChecked()
        )
        ...
        self.rhythm_diagnostic_button.setEnabled(
            invalidating_enabled
            and self._candidate_count > 0
            and not self._rhythm_diagnostic_busy
        )
        self.rhythm_profile_combo.setEnabled(
            invalidating_enabled
            and self._candidate_count > 0
            and not self._rhythm_diagnostic_busy
        )
