...
CONTOUR_VERTICAL_SUPERSAMPLING = 4
CONTOUR_HORIZONTAL_SUPERSAMPLING = 2
...
def _open_layer(
    ...
    if loader is not None:
        ...
        if value is None:
            ...
            try:
                from bdo_music_composer.transcription.bdo_transcription import load_transcription_evidence
...
def _open_time_values(source: object | None) -> tuple[np.ndarray | None, Callable[[], None]]:
    if isinstance(source, _CachedFrameTimes):
        try:
            from bdo_music_composer.transcription.bdo_transcription import load_transcription_frame_times
...
def _contour_rgba_image(
    ...
    Basic Pitch contour values form a dense posterior matrix.  Painting that
    matrix directly creates a blocky heat map and joins unrelated harmonics
    into vertical spikes.  This renderer keeps several local peaks per time
    column, connects only nearby peaks, and leaves uncertain gaps open.
    """

    values = np.nan_to_num(matrix, nan=0.0, posinf=1.0, neginf=0.0)
    values = np.clip(values, 0.0, 1.0)
    width, pitch_bins = values.shape
    render_width = width * CONTOUR_HORIZONTAL_SUPERSAMPLING
    render_height = pitch_bins * CONTOUR_VERTICAL_SUPERSAMPLING
    if width <= 0 or pitch_bins <= 0:
        return QImage()

    profile = str(denoise_profile)
    if profile not in CONTOUR_DENOISE_PROFILES:
        profile = "standard"
    (
        threshold,
        maximum_peaks,
        minimum_persistence_ms,
        bridgeable_gap_ms,
        maximum_join_bins,
    ) = CONTOUR_DENOISE_PROFILES[profile]
    minimum_run_columns = max(
        2,
        math.ceil(minimum_persistence_ms / TILE_DURATION_MS * width),
    )
    maximum_gap_columns = max(
        1,
        math.ceil(bridgeable_gap_ms / TILE_DURATION_MS * width),
    )

    def image_point(x: int, pitch_bin: int) -> tuple[float, float]:
        return (
            (x + 0.5) * CONTOUR_HORIZONTAL_SUPERSAMPLING - 0.5,
            render_height
            - 1
            - ((pitch_bin + 0.5) * CONTOUR_VERTICAL_SUPERSAMPLING - 0.5),
        )
    ...
    for x in range(width):
        column = values[x]
        peak_bins = np.flatnonzero(local_maxima[x])
        if peak_bins.size > maximum_peaks:
            strongest = np.argpartition(
                column[peak_bins], -maximum_peaks
            )[-maximum_peaks:]
            peak_bins = peak_bins[strongest]
        current = sorted(
            ((int(pitch_bin), float(column[pitch_bin])) for pitch_bin in peak_bins),
            key=lambda item: (-item[1], item[0]),
        )
        active_track_ids = [
            track_id
            for track_id in active_track_ids
            if x - tracks[track_id][-1][0] <= maximum_gap_columns + 1
        ]
        unused_tracks = set(active_track_ids)
        for pitch_bin, strength in current:
            compatible = [
                track_id
                for track_id in unused_tracks
                if abs(tracks[track_id][-1][1] - pitch_bin)
                <= maximum_join_bins
            ]
            if compatible:
                track_id = min(
                    ...
                    key=lambda candidate_id: (
                        abs(tracks[candidate_id][-1][1] - pitch_bin),
                        x - tracks[candidate_id][-1][0],
                        -tracks[candidate_id][-1][2],
                        candidate_id,
                    ),
                )
