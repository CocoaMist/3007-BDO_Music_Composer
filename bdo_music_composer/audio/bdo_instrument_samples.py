...
# The recovered v145 HIRC map contains a systematic C4-boundary root-note
# discontinuity.  Keep the authored values in the evidence JSON/TSV, but do
# not let preview multiply those samples by an extra one to four octaves.
# Each override is deliberately scoped by bank, key zone, and authored root;
# broad heuristics would damage legitimate recorder/flute sample reuse and
# native harmonic routes.
CONFIRMED_ROOT_NOTE_OVERRIDES: dict[
    str,
    dict[tuple[int, int, int], int],
] = {
    "midi_instrument_00_acousticguitar": {(49, 60, 48): 60},
    "midi_instrument_06_harp": {(60, 71, 48): 60},
    "midi_instrument_07_piano": {(60, 71, 48): 60},
    "midi_instrument_08_violin": {(49, 60, 36): 60},
    "midi_instrument_10_proguitar": {(57, 60, 48): 60},
    "midi_instrument_11_proflute": {(57, 60, 48): 60},
    "midi_instrument_14_probasselectric": {(59, 64, 48): 60},
    "midi_instrument_15_probasscontra": {
        (59, 62, 48): 60,
        (59, 64, 48): 60,
    },
    "midi_instrument_16_proharp": {
        (55, 60, 12): 60,
        (60, 61, 12): 60,
        (60, 61, 24): 60,
    },
    "midi_instrument_17_propiano": {(57, 60, 48): 60},
    "midi_instrument_18_proviolin": {
        (57, 62, 48): 60,
        (58, 63, 48): 60,
    },
    "midi_instrument_24_proguitarelectricclean": {(57, 60, 48): 60},
    "midi_instrument_25_proguitarelectricdrive": {(57, 60, 48): 60},
    "midi_instrument_26_proguitarelectricdist": {(57, 60, 48): 60},
    "midi_instrument_27_proclarinet": {(57, 60, 48): 60},
    "midi_instrument_28_prohorn": {(57, 60, 48): 60},
}
EXPECTED_CONFIRMED_ROOT_NOTE_OVERRIDE_ROWS = 116
...
def effective_sample_root_note(bank: str | None, row: dict) -> int:
    """Return the validated preview root while preserving raw HIRC evidence."""
    ...
    return CONFIRMED_ROOT_NOTE_OVERRIDES.get(str(bank), {}).get(
        signature,
        authored_root,
    )


def sample_pitch_ratio(
    row: dict,
    target_pitch: int | float,
    *,
    bank: str | None = None,
) -> float:
    ...
    remain audible and consistent in real-time and offline paths.  Confirmed
    C4-boundary HIRC discontinuities use a bank-scoped effective root without
    rewriting the checked-in evidence map.
    """
    random_midpoint = (
    ...
    cents = (
        (
            float(target_pitch)
            - float(effective_sample_root_note(bank, row))
        )
        * 100.0
...
def _routed_zone_rows(
    rows: list[dict] | tuple[dict, ...],
    ntype: int,
) -> list[dict]:
    """Return usable rows after applying the verified articulation route."""
    ...
    if route_rows:
        return route_rows
    # A bank with verified routes must not fall through into a different
    # articulation merely because the requested route is absent.
    return [row for row in usable if not _row_ntypes(row)]


def velocity_zone_boundaries(
    rows: list[dict] | tuple[dict, ...],
    pitch: int,
    ntype: int = 0,
    *,
    ...
    """Return authored Wwise velocity-zone transitions for one routed pitch.

    These values describe mapping structure, not an independently verified
    loudness or timbre threshold in the running game.
    """
    def signature(velocity: int) -> tuple[int, tuple[int, ...]] | None:
        variants = select_zone_variants(
            rows, int(pitch), velocity, ntype, bank=bank
        )
        if not variants:
            return None
        first = variants[0]
        ...
        return group_id, tuple(sorted(int(row["source_id"]) for row in variants))

    previous = signature(0)
    boundaries: list[int] = []
    for velocity in range(1, 128):
        current = signature(velocity)
        if current != previous and current is not None and previous is not None:
            boundaries.append(velocity)
        previous = current
...
def select_zone_variants(
    ...
    *,
    ...
    """
    usable = _routed_zone_rows(rows, ntype)
    ...
    def group_score(item: tuple[int, list[dict]]) -> tuple[float, float, int]:
        ...
        root_distance = min(
            abs(int(pitch) - effective_sample_root_note(bank, row))
