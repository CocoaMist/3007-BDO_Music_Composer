...
- Promote the complete editor, preview, persistence, validation, performance,
  localization, and release-safety work to the first stable major release.
- Clear the v1.0.0 public build for its deterministic schema-2 dependency
  inventory, include ONNX Runtime's complete `ThirdPartyNotices.txt`, and keep
  future dependency changes fail-closed behind a new digest review.
