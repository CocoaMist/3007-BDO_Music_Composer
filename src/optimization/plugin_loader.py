...
from .plugin_api import (
    ...
    PluginEnvironment,
    VALID_SCOPES,
    negotiate_optimizer_capabilities,
)
...
def _manifest(payload: dict[str, Any]) -> OptimizerBundleManifest:
    ...
    if len(set(capabilities)) != len(capabilities):
        raise OptimizerBundleError("plugin capabilities contain duplicates")
    try:
        negotiated = negotiate_optimizer_capabilities(frozenset(capabilities))
    except ValueError as exc:
        raise OptimizerBundleError(str(exc)) from exc
    return OptimizerBundleManifest(
        ...
        tuple(sorted(negotiated.capabilities)), bool(payload["requires_safe_prepass"]),
    )
...
def load_optimizer_bundle(descriptor: OptimizerBundleDescriptor) -> tuple[object, PluginEnvironment]:
    ...
    return plugin, PluginEnvironment(
        root,
        frozenset(descriptor.manifest.capabilities),
    )
