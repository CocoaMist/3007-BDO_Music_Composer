...
creates formal editor notes. The projection retains three internal levels of
detail, but the production canvas deliberately shows only short inter-note
bridges at phrase/detail zoom. Candidate blocks already communicate duration
and pitch; repeating their plateaus or drawing a full-song overview contour
creates a distracting second melody.
...
def melody_line_width(value: object) -> float:
    """Encode confidence without letting advisory lines dominate notes."""

    return 0.70 + melody_line_confidence_bucket(value) * 0.12
...
def melody_line_kind_visible(
    ...
    *,
    ...
    if lod <= OVERVIEW_LOD:
        return False
    if normalized in {CONTOUR_KIND, NOTE_KIND}:
        return False
    if branch and lod < DETAIL_LOD:
        return False
    return normalized in {CONNECTOR_KIND, CHORD_SPAN_KIND}
...
def _append_overview_contour(
    ...
    *,
    ...
    maximum_gap = max(80.0, min(220.0, beat_ms * 0.35))
    ...
    for phrase in phrases:
        ...
        for previous, current in zip(points, points[1:]):
            if abs(previous[1] - current[1]) > 7.0:
                continue
            output.append(
                MelodyLineSegment(
                    role,
...
def _append_path_segments(
    ...
    *,
    ...
    maximum_gap = max(80.0, min(220.0, float(beat_ms) * 0.35))
    for record in ordered:
        ...
        if (
            ...
            and abs(record.pitch - previous.pitch) <= 7
        ):
            output.append(
                MelodyLineSegment(
                    role,
