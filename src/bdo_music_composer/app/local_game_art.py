"""Import two allow-listed composition-art resources into a local cache.
...
The minimal PAZ path-table primitives live in the Qt-free application core.
They expose no archive browser or audio extraction command. Unknown archive
versions, paths, compression modes, sprite layouts, and oversized inputs fail
closed.
"""
...
def import_game_instrument_art(
    ...
    cache_root: str | Path,
    *,
    tile_writer: Callable[
        [bytes, SpriteLayout, Path],
    ...
    try:
        images = tile_writer(sprite_payload, layout, staging)
