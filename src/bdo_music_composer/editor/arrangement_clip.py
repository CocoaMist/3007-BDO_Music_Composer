    ...
    for clip in track_clips(track):
        for note in track.notes:
            if not _note_belongs_to_clip(note, clip):
                continue
            start = max(float(note.start), clip.start_ms)
            end = min(_note_end(note, scale), clip.end_ms)
            if end - start >= 1.0:
                projected.append(_note_with_times(
                    note, start=start, duration=end - start
                ))
    return tuple(sorted(
        projected,
    ...
    for clip in track_clips(track):
        for record in track.bdo_source_note_records:
            ...
            if not clip.content_start_ms <= onset < clip.content_end_ms:
                continue
            start = max(onset, clip.start_ms)
            end = min(onset + max(1.0, float(record[3]) * scale), clip.end_ms)
            if end - start < 1.0:
                continue
            value = list(deepcopy(tuple(record)))
            value[2], value[3] = start, end - start
...
def plan_clip_edit(
    ...
    *,
    ...
    moved_clip = replace(
        ...
        start_ms=start,
        end_ms=end,
        content_start_ms=clip.content_start_ms + delta,
        content_end_ms=clip.content_end_ms + delta,
    )
    ...
    moved_notes = tuple(
        note._replace(start=max(0.0, float(note.start) + delta))
        for note in source.notes if in_content(float(note.start))
    )
    kept_notes = tuple(
        deepcopy(note) for note in source.notes if not in_content(float(note.start))
    )
    moved_controls = tuple(
        _mapping_time(value, lambda time: time + delta)
    ...
    kept_controls = tuple(
        ...
        if not in_content(float(value.get("time", -1.0)))
    )
    moved_records = tuple(
        _record_time(value, lambda time: time + delta, 1.0)
    ...
    kept_records = tuple(
        ...
        if not in_content(float(value[2]))
    )
    ...
    if destination is source:
        ...
        return ClipEditPlan((replace(
            update,
            notes=tuple(sorted((*kept_notes, *moved_notes), key=lambda note: note.start)),
            performance_controls=tuple(sorted((*kept_controls, *moved_controls), key=lambda value: float(value.get("time", 0.0)))),
            source_note_records=tuple(sorted((*kept_records, *moved_records), key=lambda value: float(value[2]))),
            arrangement_clips=_replace_clip(
...
def plan_clip_split(
    ...
    left = replace(
        ...
        clip_id=left_id,
        end_ms=split,
        content_end_ms=split,
    )
    right = replace(
        clip,
        clip_id=right_id,
        start_ms=split,
        content_start_ms=split,
    )
    notes: list[Note] = []
    for note in track.notes:
        onset, note_end = float(note.start), _note_end(note)
        if (
            clip.content_start_ms <= onset < split < note_end
            and onset < clip.content_end_ms
        ):
            notes.append(note._replace(dur=split - onset))
            notes.append(note._replace(start=split, dur=note_end - split))
        else:
            notes.append(deepcopy(note))
    records: list[tuple] = []
    for record in track.bdo_source_note_records:
        onset = float(record[2])
        record_end = onset + float(record[3])
        if clip.content_start_ms <= onset < split < record_end:
            left_record = list(deepcopy(tuple(record)))
            right_record = list(deepcopy(tuple(record)))
            left_record[3] = split - onset
            right_record[2] = split
            right_record[3] = record_end - split
            records.extend((tuple(left_record), tuple(right_record)))
        else:
            records.append(deepcopy(tuple(record)))
    update = replace(
        _base_update(track),
        notes=tuple(sorted(notes, key=lambda note: (note.start, note.pitch))),
        source_group_index=None,
        source_note_records=tuple(sorted(records, key=lambda value: float(value[2]))),
